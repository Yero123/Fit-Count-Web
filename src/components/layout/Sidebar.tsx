...
const Sidebar = () => {
  ...
  return (
    <div className='hidden md:flex min-w-[150px] px-10 bg-[#FAFAFA] h-[100vh] flex-col dark:bg-[#02081B]'>
      <Link href="/" className='pl-4 font-bold text-[1.6rem] md:p-0 md:text-[2rem] mt-6 dark:text-white'><h1>Fit count</h1></Link>
      ...
      <div className='mt-12 flex flex-col gap-6 min-w-[200px]'>
        {
          rutines.map((rutine: any, index: any) => {
            const hasExerecises = rutine.exercises.some((exercise: any) => router.asPath === `/exercises/${exercise.id}`)
            const isActive = router.asPath === `/rutines/${rutine.id}` || hasExerecises
            return (
              <details key={index} className='flex gap-3'>
                <summary className='flex '>
                  ...
                  <Link href={`/rutines/${rutine.id}`}>
                    <p className={`font-bold text-lg self-center ${isActive ? "dark:text-primary" : " dark:text-slate-400"} 
                    ${isActive ? "text-slate-900" : " text-slate-500"}
                     hover:text-slate-900 dark:text-slate-400 dark:hover:text-white `}>{rutine.name}</p>
                  </Link>
                </summary>
                <div className='flex flex-col gap-3 pt-3'>
                  {
                    rutine.exercises.map((exercise: any, index: any) => {
                      const isActive = router.asPath === `/exercises/${exercise.id}`
                      return (
                        <details key={index} className='flex gap-3'>
                          <summary className='flex pl-8 gap-3'>
                            ...
                            <Link href={`/exercises/${exercise.id}`}>
                              <p className={`font-bold text-base self-center ${isActive ? "dark:text-primary" : " dark:text-slate-400"}  ${isActive ? "text-slate-900" : " text-slate-500"} text-slate-500 hover:text-slate-900  dark:hover:text-white `}>{exercise.name}</p>
                            </Link>
                          </summary>
                        </details>
                      )
...
const SidebarItem = ({ title, icon, active, href }: {
  ...
  return (
    <div className='flex gap-3'>
      <div className={`rounded-full ${active ? "bg-primary" : ""} h-7 w-7  flex items-center justify-center `} >
        {
          icon
        }
      </div>
      <Link href={href}>
        <p className={`font-bold text-lg self-center  hover:text-slate-900 ${active ? "text-slate-900" : "text-slate-500"}
          ${active ? "dark:text-white" : "dark:text-slate-500"}  dark:hover:text-white 
        `}>{title}</p>
      </Link>
    </div>
  )
